...
export default function CheckinTab() {
  ...
  const scan = useMutation({
    ...
    onSuccess: (res) => {
      ...
      if (res.result === 'SUCCESS') toast.push('Checked in.', 'success');
    },
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <div className="space-y-4">
        ...
        {outcome && (
          <Card
            className={`border-l-4 ${outcome.result === 'SUCCESS' ? 'border-l-status-success' : outcome.result === 'DUPLICATE' || outcome.result === 'WRONG_SESSION' ? 'border-l-status-warning' : 'border-l-status-error'}`}
          >
            ...
            {outcome.ticket && (
              <div className="mt-3 space-y-1 text-sm">
                <p className="font-medium text-text-primary">{outcome.ticket.holderName ?? '—'}</p>
