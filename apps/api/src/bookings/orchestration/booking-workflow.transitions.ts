...
export const ALLOWED_TRANSITIONS: Readonly<Record<S, ReadonlySet<S>>> = {
  ...
  [S.LOCKED]: new Set([
    S.PAYMENT_PENDING, // local / allocated inventory
    S.PROVIDER_RESERVATION_PENDING, // provider-authoritative inventory
    S.EXPIRING,
    S.CANCELLATION_PENDING,
    S.COMPENSATION_PENDING,
    S.FAILED,
  ]),
  [S.PROVIDER_RESERVATION_PENDING]: new Set([
    S.PROVIDER_RESERVED,
    S.EXPIRING,
    S.CANCELLATION_PENDING,
    S.COMPENSATION_PENDING,
