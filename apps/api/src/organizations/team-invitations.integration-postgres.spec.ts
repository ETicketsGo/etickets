...
describe('integration-real-postgres: team invitations', () => {
  ...
  afterAll(async () => {
    ...
    await db.organizationInvitation.deleteMany({ where: { member: { organizationId: orgId } } });
  ...
  maybe(
    ...
    async () => {
      ...
      */
      ...
      const row = await db!.organizationInvitation.findUnique({
        where: { memberId: invited.id },
      });
