...
from events: a film is a reusable catalogue entity that screens as many *shows*
across *screens* in *cinemas*, and (PR-3) is booked by selecting specific seats.
...
- `Cinema` — an org's cinema; optionally linked to a `Venue` for city/discovery
  reuse. (We fold the spec's "Multiplex" into `Cinema` — a cinema *is* the
...
- `EventSession.screenId?` → the screen a *show* plays on.
...
— only the *inventory model* differs, and that is already pluggable (ADR-010).
